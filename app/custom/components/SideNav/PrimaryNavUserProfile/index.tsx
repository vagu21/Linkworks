        ...
        >
          {(() => {
            if (user?.avatar) {
              ...
              return <span className="inline-block h-5 w-5 overflow-hidden rounded-full text-[#FFFFFF]">{user?.firstName?.[0] || "U"}</span>;
            }
          })()}
        </button>
      </div>
          ...
          {opened &&
            createPortal(
              <div className="divide-border fixed bottom-[20px] left-[85px] z-50 mt-2 w-64 origin-top-right divide-y overflow-hidden rounded-sm border border-gray-200 bg-[#FFFFFF] shadow-lg focus:outline-none">
                <div className="shadow-xs rounded-sm bg-[#FFFFFF] py-1" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                  <div
                    className="text-muted-foreground group flex items-center truncate px-2.5 py-2 text-sm transition duration-150 ease-in-out"
                    role="menuitem"
                  >
                    <div className="flex gap-2 truncate">
                      <div className=" divide-x divide-gray-300 rounded-sm shadow-none">
                        ...
                        >
                          {(() => {
                            if (user?.avatar) {
                              return (
                                <img className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-800" src={user.avatar} alt="Avatar" />
                              );
                            } else {
                              return (
                                <span className="inline-block h-5 w-5 overflow-hidden rounded-full text-[#FFFFFF]">{`${user?.firstName?.[0] || ""}${user?.lastName?.[0] || ""
                                  }`}</span>
                              );
                            }
                          })()}
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="truncate font-light text-[#737373]">{appOrAdminData.user?.email}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  {layout === "app" ? (
                    <>
                      <Link
                        className=" text-[#737373] block px-4 py-2 text-sm transition duration-150 ease-in-out hover:bg-[#FFEFC9] group mx-1 items-center gap-2.5 rounded-md mb-1  mt-2"
                        role="menuitem"
              ...
              >
                {t("models.group.plural")}
                      ...
                      <div className="mx-1 mb-2 mt-1 border-t border-[#E6E6E6]"></div>
                      <span className="mx-1 !py-3 px-3 text-[14px] text-[#4D4D4D]">Switch Account</span>
                          ...
                          {appOrAdminData.myTenants.map((tenant) => {
                            ...
                            return (
                              <Link
                                key={tenant.id}
                                className={clsx(
                                  "group mx-1 mb-1 mt-2 flex items-center gap-2.5  rounded-md",
                              ...
                              >
                                <div className=" group mx-1 flex items-center gap-2.5 rounded-md px-3 py-2">
                                  <span className="rounded-[4px] bg-orange-500 px-1 text-[12px] text-[#FFFFFF]">{tenant.name[0]}</span>
                                  <span className="text-sm text-[#737373] group-hover:text-[#262626]">{tenant.name}</span>
                                </div>
                              </Link>
                            );
                          })}
                          <Link to={"/new-account"} role="menuitem" onClick={closeDropdownUser}>
                            <div className="hover:bg-sub-menu-background-selected group mx-1 mb-[4px] flex items-center gap-2 rounded-md px-3 py-2 text-[14px] text-[#737373]">
                            ...
                            <div className="mx-1 border-t border-[#E6E6E6]"></div>
                          </Link>
                        </>
                      )}
                    </>
                    ...
                    <Link
                      className=" block px-4 py-2 text-sm text-[#737373] transition duration-150 ease-in-out"
                      role="menuitem"
                      onClick={closeDropdownUser}
                  ...
                  {!items && (
                    <Link
                      to="/logout"
                      // onClick={signOut}
                      className=" flex w-full gap-2.5 px-4 py-2 text-left text-sm text-[#FE7070] transition duration-150 ease-in-out hover:bg-[#FEF1F0] focus:outline-none"
                      role="menuitem"
                      // disabled={!user}
                    >
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
