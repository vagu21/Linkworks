...
const DashboardCharts = ({ data }: { data: LoaderData }) => {
    ...
    const prepareJobMetrics = (data: LoaderData) => {
        const jobNumericData = [{ status: "Open", job: data.dashboardStatsData.Jobs.openJobsCount, fill: "hsl(var(--chart-1))" },
        ...
        <>
            <div className='flex flex-col gap-4 pb-4'>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    <div><StatCard charTitle={"Open to Work Candidates"} charSubTitle={subTitle} chartDescription={`Total candidates open to work`} numericData={data.dashboardStatsData.Candidates.openToWorkCandidatesCount} /></div>
                    <StatCard charTitle={"Open Jobs"} charSubTitle={subTitle} chartDescription={`Total Open Jobs`} numericData={data.dashboardStatsData.Jobs.openJobsCount} />
                    <StatCard charTitle={"Pending Job Applications"} charSubTitle={subTitle} chartDescription={`Total Pending Job Applications`} numericData={data.dashboardStatsData.JobApplications.pendingJobApplicationsCount} />
                    <StatCard charTitle={"Accounts"} charSubTitle={subTitle} chartDescription={`Total Accounts`} numericData={data.dashboardStatsData.Account.accountsCount} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-[2fr_1fr] gap-4">
                   <div> {jobMetrics.length && <BarChartComponent chartData={candidateMetrics} charTitle={"Candidates"} charSubTitle={subTitle} chartDescription={`Showing total Candidates for ${subTitle}`} />}</div>
                    {candidateMetrics.length && <PieDonutChart chartData={jobMetrics} charTitle={"Jobs"} chartSubTitle={subTitle} chartDescription={`Showing total Jobs for ${subTitle}`} />}
                </div>

                {lineChartData.length && <LineChartComponent data={lineChartData} charTitle={"Job Applications and Candidates Trend"} charSubTitle={""} chartDescription={`Job Applications and Candidates Trend`} />}
            </div>
        </>
    )
}
