const transformCandidateData = (response: any, row: any) => {
  console.log("transformCandidateData", response);
  const skills: any[] = [];
  // const skillRatingMapping: Record<string, string> = {};
  const frequencyMap: Record<string, number> = {};

  const workExperience = Array.isArray(response["Work Experience"])
    ? response["Work Experience"]?.length
  ...
  const EducationHistory = Array.isArray(response["Education History"])
    ? response["Education History"]?.length
    ...
    : response["Education History"]
      ...
      const Certifications = Array.isArray(response["Certification"])?
      response["Certification"]?.length
  ...
  // }
      ...
      const skillMapping = response?.Skills.map((skill: any) => {
        skillRatingMapping[skill.skillName] = skill.proficiency;
  ...
  const sortedSkills = skillMapping.sort((a: any, b: any) => b.proficiency - a.proficiency);

  let topSkills = sortedSkills.slice(0, 7);

  topSkills = topSkills.map((skill: any) => skill.skillName);

   console.log("topSkills", topSkills,skillRatingMapping);
  ...
  //     let rating = "95%"; // Default rating for count > 6
  //     switch (count) {
  //       case 1:
  //         rating = "45%";
  //         break;
  ...
  //         break;
  //     }
  //     skillRatingMapping[skill] = rating;
  //   });
  // }

  const totalExperienceInYears = response.totalExperienceInYears;
  const currentDesignation = response.currentDesignation;
...
const apiKey: string | undefined = import.meta.env.VITE_RESUME_API_KEY;

const fetchCandidate = async (candidate: any) => {
  const response = await fetch(`http://works.lfiapps.com/api/candidates/${candidate.id}`, {
    headers: {
  ...
  if (response.ok && response.status == 200) {
    ...
    if (jsonResponse?.data) {
      return transformCandidateData(jsonResponse.data, candidate);
    }
  } else {
...
export async function generateResume(rowData: any) {
  const { skills: skillArray, skillRatingMapping, ...data } = await fetchCandidate(rowData);

  const DocumentHeader = (): string => {
    return /* HTML */`
      <meta charset="UTF-8" />
  ...
  const CompanyHeader = (): string => {
    return /* HTML */`
      <!-- New Header -->
      <header class="flex justify-center gap-x-2 bg-gray-800 py-4 text-center text-white">
        <svg width="37" height="41" viewBox="0 0 37 41" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M0 40.6009H6.51259C12.5077 40.6009 17.3691 35.5741 17.3691 29.3742V0.744141H10.8565C4.85921 0.744141 0 5.77096 0 11.9708V40.6009Z"
            fill="#FFD600"
          />
          <path d="M36.7655 0.858398V8.21511C36.7655 14.4661 32.1919 19.5347 26.549 19.5347H19.3965V0.858398H36.7655Z" fill="#FF7800" />
          <path d="M36.7655 22.0957V29.4524C36.7655 35.7034 32.1919 40.772 26.549 40.772H19.3965V22.0957H36.7655Z" fill="#2D58C0" />
        </svg>
        <h1 class="text-3xl font-bold">Linkfields Innovations</h1>
      </header>
    `;
  }

  const CompanyFooter = (): string => {
    return /* HTML */`
      <footer class="flex justify-center gap-x-2 bg-gray-800 py-4 text-center align-bottom text-white">
        <p>&copy; 2024</p>
        <svg width="17" height="21" viewBox="0 0 37 41" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M0 40.6009H6.51259C12.5077 40.6009 17.3691 35.5741 17.3691 29.3742V0.744141H10.8565C4.85921 0.744141 0 5.77096 0 11.9708V40.6009Z"
            fill="#FFD600"
          />
        ...
        <p>Linkfields Innovations. All rights reserved.</p>
      </footer>
    `
  }
  ...
  const SkillsSection = (): string => {
    if (skillArray?.length) {
      return /* HTML */ `<section class="mt-8">
      ...
      <p class="text-gray-600 ml-auto justify-end"> ${skillRatingMapping[skill]}</p>
      </div>
     
         <div class="skill-bar">
          <div class="skill-fill" style="width: ${skillRatingMapping[skill]};"></div>
          </div>
      `
          )
          .join("")}
    ...
    if (data.workExperience?.length) {
      return /* HTML */ `
        ...
        (exp: any) => `
                <div class="space-y-4">
                  <div>
                    <h3 class="font-semibold">${exp["title"]} at ${exp["companyName"]}</h3>
                    <p class="text-gray-600">${new Date(exp["startDate"]).toLocaleDateString()} - ${exp["currentlyWorkingInThisRole"] ? "Present" : new Date(exp["endDate"]).toLocaleDateString()}</p>
            ...
            .map((line: any, index: number, arr: any[]) => {
              ...
              return line ? `<li><div class="mb-2">${line}.</div></li>` : "";
            })
            .join("")}
                    </ul>
                  </div>
  ...
  const EducationHistory = (): string => {
    if (data?.['Education History']) {
      const educationHistory = data["Education History"];
      return /* HTML */`
        <section class="mt-8">
          ...
          <div class="space-y-4">
            <div>
              <h3 class="font-semibold text-lg text-gray-800">
                ${educationHistory.educationQualification || 'N/A'} 
                ${educationHistory.educationalSpecialization || ''}
              </h3>
              <p class="text-gray-600">
                ${educationHistory.schoolCollegeName || 'N/A'}
              </p>
              <p class="text-gray-500">
                ${educationHistory.startDate
          ? new Date(data["Education History"].startDate).toLocaleDateString()
          : 'Start Date N/A'} 
                - 
                ${educationHistory.endDate
          ? new Date(data["Education History"].endDate).toLocaleDateString()
          : 'End Date N/A'}
              </p>
            </div>
          </div>
    ...
    <div class="mx-auto   rounded-lg bg-white shadow-md h-[100%] w-[100%]">
      ...
      <div class="flex items-center justify-between border-b bg-[#EFF2F8] py-6 pl-6 pr-7">
        <div class="flex items-center gap-[8px]">
          <div class="flex h-[45px] w-[45px] items-center justify-center rounded-full bg-[#18213F] text-[16px] font-medium leading-[19.36px] text-white">${data['firstName'][0]}${data['lastName'][0]}</div>
...
<!--   <div class="absolute inset-[10%] bg-[#EFF2F8] rounded-full"></div>
  <span class="text-[#28952E] text-sm font-semibold relative z-20">
    75%
      ...
      <!-- Details Section -->
      <div class="flex  flex-col px-6 md:flex-row  relative ml-8">
        <!-- Left Section -->
...
<path d="M6.25 8.74995V5.41661C6.25 5.30611 6.2061 5.20012 6.12796 5.12198C6.04982 5.04384 5.94384 4.99995 5.83333 4.99995H4.16667C4.05616 4.99995 3.95018 5.04384 3.87204 5.12198C3.7939 5.20012 3.75 5.30611 3.75 5.41661V8.74995M1.25 4.16661C1.24997 4.04539 1.27639 3.92562 1.32741 3.81566C1.37843 3.7057 1.45283 3.60819 1.54542 3.52995L4.46208 1.03036C4.61249 0.903241 4.80307 0.833496 5 0.833496C5.19693 0.833496 5.38751 0.903241 5.53792 1.03036L8.45458 3.52995C8.54717 3.60819 8.62157 3.7057 8.67259 3.81566C8.72361 3.92562 8.75003 4.04539 8.75 4.16661V7.91661C8.75 8.13763 8.6622 8.34959 8.50592 8.50587C8.34964 8.66215 8.13768 8.74995 7.91667 8.74995H2.08333C1.86232 8.74995 1.65036 8.66215 1.49408 8.50587C1.3378 8.34959 1.25 8.13763 1.25 7.91661V4.16661Z" stroke="#8E8E8E" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round"/>
                 ...
                 <span class="ml-5">${data.Address?.flatHouseNoBuildingCompanyApartment}, ${data?.Address?.landmark}, ${data.Address?.townCity}, ${data.Address?.state}, ${data.Address?.country} </span></li>
            </ul>
          </div>
          <!-- Skills -->
          <div class="mb-[30px]">
            ...
            <ul>
            ${data?.topSkills?.map((skill: any) => `<li class="mb-[12px]">        <div class="flex justify-between"><span class="text-[12px] font-medium leading-[16px] text-[#000000]">${skill}</span>  <span class="text-[10px] font-normal leading-[11.52px] text-[#000000]">${proficiencyMapping[skillRatingMapping[skill]]}</span>    </div><div class="mt-[6px] w-full rounded bg-gray-300">
                 ${skillBarMapping[skillRatingMapping[skill]]}
                </div></li>`).join("")}
        ...
        <div class="flex-1 py-6 md:pl-6">
          ...
          <div class="mb-[30px]">
            ...
            ${data?.workExperience?.map((exp: any) => `<li class="text-[10px] font-normal leading-[11.52px] text-[#A3A5A8]">
                <div class="flex items-center gap-[10px]">
                  <strong class="text-[12px] font-medium leading-[14.52px] text-[#1C1C1C]">${exp?.title}</strong>
                  <p class="text-[10px] font-normal leading-[11.52px] text-[#A3A5A8]">· ${new Date(exp['startDate']).toLocaleDateString()}-  ${exp['currentlyWorkingInThisRole'] ? 'Present' : new Date(exp['endDate']).toLocaleDateString()}</p>
                </div>
          ...
          <div class="mb-[30px]">
            ...
            <ul class="space-y-4">
            ${data?.Certifications?.map((certification: any) => `<li >
                <div class="flex items-center gap-[10px]">
                  <strong class="text-[12px] font-medium leading-[14.52px] text-[#1C1C1C]">${certification?.name}</strong>
                  <p class="text-[10px] font-normal leading-[11.52px] text-[#A3A5A8]">· Valid till ${new Date(certification['expirationDate']).toLocaleDateString()}</p>
                </div>
          ...
          <div class="mb-[30px]">
            ...
            <ul class="space-y-4">
            ${data['Education History']?.map((education: any) => `<li >
                <div class="flex items-center gap-[10px]">
                  <strong class="text-[12px] font-medium leading-[14.52px] text-[#1C1C1C]">${education?.educationQualification}, ${education?.educationalSpecialization}</strong>
                    <p class="text-[10px] font-normal leading-[11.52px] text-[#A3A5A8]">· ${new Date(education['startDate']).toLocaleDateString()} - ${new Date(education['endDate']).toLocaleDateString()}</p>
                </div>
        ...
        <!-- background image -->
    <div class="absolute top-[50%] translate-y-[-50%] left-[37%]">
...
<rect width="194.99" height="195" fill="white"/>
      ...
      <!-- Footer -->
      <div class="flex flex-col items-center justify-between border-t bg-[#D6DCE8] px-6 py-4 text-center md:flex-row">
        <!-- 
...
export async function downloadResumeAction(rowData: any) {
  let resumeHTML = await generateResume(rowData) || '';
  const response = await fetch("/api/generate-pdf", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Api-Key": "9f254622-d88a-4125-ac99-85f2b8cf2173",
    },
