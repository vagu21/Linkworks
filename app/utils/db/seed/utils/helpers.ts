...
export async function seedTemplateRoles({ templateName, templatePermissions, templateSpecificRoles }: SeedTemplateRolesArgs) {
  ...
  const getEntityPermissionMapByRole = async (role: string) => {
    const rolePermissionsMap = new Map(templatePermissions.filter((o) => o.inRoles.includes(role)).map((o) => [String(o.name), o]));
